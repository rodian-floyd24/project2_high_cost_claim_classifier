...
Four supervised models were evaluated for beneficiary-level next-year high-cost prediction: logistic regression, random forest, fixed-spec gradient boosting, and XGBoost. All models were trained on the same beneficiary-level split strategy and were evaluated on a held-out test set with approximately 10% positive-class prevalence. Performance was assessed using both global discrimination metrics and operational ranking metrics, including top-k capture and lift.
...
Gradient boosting produced the strongest overall out-of-sample ranking performance. On the latest held-out test run, it achieved an accuracy of 0.8533, precision of 0.3329, recall of 0.4769, AUC-ROC of 0.8099, and AUC-PR of 0.3879. More importantly for a risk-targeting use case, it identified 25.11% of all true high-cost beneficiaries within the top 5% highest-risk predictions and 38.77% within the top 10%. These results correspond to lifts of 5.02 and 3.88 over random selection, respectively.
...
XGBoost produced the highest recall, but not the best balanced performance. On the test set, it achieved accuracy of 0.7131, precision of 0.2179, recall of 0.7145, AUC-ROC of 0.7989, and AUC-PR of 0.3617. Its top-5% and top-10% capture rates were 23.22% and 36.91%, with lifts of 4.64 and 3.69. This profile makes XGBoost useful as a high-sensitivity triage alternative when false positives are acceptable, but it is weaker than gradient boosting on ROC-AUC, PR-AUC, top-k capture, precision, and accuracy.

Logistic regression remained valuable as an interpretable baseline and had the strongest plain accuracy in the canonical test export. It achieved accuracy of 0.8603, precision of 0.3432, recall of 0.4469, AUC-ROC of 0.8060, and AUC-PR of 0.3634. Its top-5% and top-10% capture rates were 23.86% and 37.99%. Gradient boosting improved recall, AUC-PR, and top-k capture, though it accepted slightly lower precision and raw accuracy. As a result, logistic regression is better interpreted as a strong transparent benchmark than as the preferred production candidate for risk ranking.
